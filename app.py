...
<style>
    ...
    .source-expander > div > div[data-testid="stExpanderDetails"] {
...
# --- Sidebar ---
with st.sidebar:
    st.title("🇮🇳 MeitY AI Agent")
    st.markdown("---")
    
    st.header("📖 About")
    st.markdown("""
    This AI Agent provides answers using official MeitY documents, websites, 
    and multimedia content through advanced RAG technology.
    """)
...
# --- Main Page ---
st.title("🤖 MeitY Knowledge Base AI Agent")
st.markdown("Get instant answers about Ministry of Electronics and IT policies and initiatives.")
...
if st.session_state.latest_response:
    ...
    # Display Question
    st.markdown(f"#### Question:")
    st.info(st.session_state.latest_query)

    response_data = st.session_state.latest_response
    answer = response_data.get("answer", "No answer was generated.")
    ...
    if source_docs:
        ...
        for i, doc in enumerate(source_docs):
            ...
            # Check if the source is a local document from the build process
            is_local_doc = 'source_documents' in source.replace('\\', '/')
            
            filename = os.path.basename(source)
            
            with st.expander(f"Reference [{i+1}]: {filename}"):
                st.write(f"**Content Preview:** *{doc.page_content[:250]}...*")
                
                if is_local_doc:
                    # Construct the correct path inside the Azure mount
                    azure_file_path = os.path.join(PERSISTENT_DIR, source)
                    ...
                    if os.path.exists(azure_file_path):
                        ...
                        st.warning("Source file not available for download in the current environment.")
                
                else: # It's a web URL
                    st.link_button("View Online Source", url=source)